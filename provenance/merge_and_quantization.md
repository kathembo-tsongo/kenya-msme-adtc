...
- The adapter was merged into the base model, converted to GGUF and quantized to Q4_K_M with llama.cpp (llama-quantize built from source). The notebook `My_Offline_AI_advicer_for_kenyan_msmes.ipynb` in this folder holds the cell outputs of the first run (global_step=394). The submitted run's training log is `trainer_state.json` and `training_log.txt` in this folder; its merge, conversion and quantization followed the same steps.
