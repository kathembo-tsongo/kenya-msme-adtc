...
## Post-Gate-1 recheck of the judges' questions (September 20, 2026)

We replayed the 5 automated prompts and the 12 human-judge questions (follow-ups as live multi-turn chains) two ways: directly against the GGUF through llama-server, which is the path the official profiler exercises, and through the full application (rag_server.py). The model under test is the submitted v6 weights with the v9 digest (sha256 e88ec139...). We do not know whether the LLM-judged prompts are run against the GGUF directly or through the application, and have asked the organizers.

**Raw GGUF.** We saw no repetition loops in 17 replayed prompts (context 8192, temperature 0.3), nor in a greedy stress test (temperature 0, context 2048, 900-token cap) over 16 judge-style prompts that include the five Gate 1 automated prompts. For comparison, the original-weights build with the same digest looped on 2 of those 16 (one prompt repeating the same line 46 times) and hit the token cap on 3, which is one reason we submit the v6 weights. This is a small sample of one run per prompt and should be read as a stress check, not a loop rate. Answer quality on this path is still uneven. The payroll question was answered with withholding-tax rates instead of the PAYE bands and no remittance deadline; the SHIF question with an invented salary threshold; the sole-proprietorship-to-company question with an invented iTax menu step; and the business-registration prompt often starts with a KRA PIN and NITA instead of business-name registration. The NSSF-at-KSh-30,000 prompt gave a wrong 900/900 employee/employer split in 1 of 21 sampled runs. The digest inside the GGUF does not contain SHIF, the payroll remittance deadlines, the company-registration forms or county permit content. We did not add them because any change to the digest changes the model file's hash and requires re-verifying the whole download chain before the deadline.

**Full application.** Through rag_server.py, several judged questions initially failed when they fell through to free generation. The payroll question failed in 6 of 6 runs (3 through the application, 3 through the raw model) with invented thresholds and wrong remittance details; the follow-ups "Are those all the regulations for this category of business?" and "is there any software that will help make me compliant" produced answers about unrelated retrieved material in 0 of 3 runs each; and the barber chain produced a wrong capital-gains claim and an invented URL. In this path we changed rag_server.py only (the model file is unchanged): payroll and employer-obligation phrasings now route to the verified employee-compliance answer, follow-up questions now include the previous question in the retrieval query, and five new verified answers were added, each written only from facts already in our verified entries (compliance software, "are those all the regulations", county relocation, barber/salon taxes, common sole-proprietorship-to-company mistakes). After these changes the judged questions are answered from verified text with no model call, confirmed in the server log. This applies only when the application is used; a direct GGUF load bypasses it.

**Limitations.** Our earlier statement that the v6+v9 stack showed no fabrication applies to the test battery it describes, not to every phrasing a judge might use. Questions that match no verified topic still go through free generation on a 1.5B-parameter model, which can produce inaccurate steps and can refer to "source material" the user never provided. The five new answers exist in English only, so Kiswahili questions that match them receive the English text. We did not address the judges' comment on speed, which is bounded by the evaluation hardware.


## Kiswahili: what the raw model does and what the application adds (September 20, 2026)

metadata.json lists Kiswahili ("sw") in language_scope. We tested eight Kiswahili questions (VAT rate, NSSF rate, business registration, turnover tax, VAT registration timing, first-employee obligations, SHIF rate, PAYE bands) two ways, and report both because they differ sharply.

**Raw GGUF (the file the official profiler loads; temperature 0, context 2048, no application layer).** Six of the eight answers came back in English, one was a two-token non-answer ("Kes 98,624.") to the business-registration question, and one was garbled Kiswahili with invented figures and no useful content (first-employee obligations). The turnover-tax answer stated the correct 1.5% rate; the VAT-rate answer stated 16% and the KES 5,000,000 threshold but then applied them to an invented turnover of KES 2.8 million, which contradicts the threshold. In the answers we read, the NSSF question got an invented salary and tier boundary, the SHIF question got a figure of "KES 25,000 per month" instead of 2.75% of gross pay, and the PAYE question got a wrong description of the 25% band. This is consistent with our earlier disclosure that our training data and retrieval corpus are English-only. We therefore do not claim that the raw model answers Kiswahili questions reliably.

**Full application (rag_server.py).** After fixes made during this test, all eight questions were answered from verified text with no model call: seven in Kiswahili (VAT, NSSF, registration, turnover tax, SHIF, PAYE, and VAT timing) and one, the first-employee question, in English, because the verified employee-compliance answer exists only in English. Before the fixes, one question (the VAT rate) was answered by free generation with the wrong base ("16% of annual taxable income"), one returned English text for a Kiswahili question because our language detector missed the words "lini" and "ninapaswa", and one (first-employee obligations) produced invented words. The fixes were: adding the 16% VAT rate to the verified VAT answers in both languages, adding VAT-rate keywords, extending the Kiswahili word list, and routing Kiswahili payroll phrasings to the verified employee-compliance answer.

**Limitations.** Kiswahili quality therefore comes from a set of verified answers in the optional application, not from the model. Kiswahili questions that match no verified topic still go to free generation, where the 1.5B model can produce English or incorrect Kiswahili. Five newer verified answers exist in English only. The Kiswahili before/after example above compares the base model with the earlier weights; the submitted v6 weights answer the same question differently on the raw path.

**Status and future work.** Kiswahili support in this submission is partial. In the optional application it works for the verified topics described above (7 of our 8 test questions were answered in Kiswahili from verified text, and the eighth in English), but that quality comes from the verified answers and not from the model. In the GGUF itself Kiswahili remains future work: the model was fine-tuned on English-only data, and the raw model mostly answers Kiswahili questions in English, sometimes with incorrect figures. We did not have time before the Gate 2 submission deadline to build a Kiswahili training set, retrain, and re-verify the model file. A new model changes the file's hash and would require re-uploading it, updating the download script and provenance files, and repeating the benchmarks, so we chose not to put the verified submission at risk. We list "sw" in the language_scope field of metadata.json to reflect the application's Kiswahili support, and describe its limits here so the claim is not read as a claim about the raw model.

Our planned next steps are: (1) build a Kiswahili training set from verified Kenyan regulatory sources with native-speaker review; (2) retrain and evaluate the raw model on Kiswahili questions, including the eight used here; and (3) add Kiswahili versions of the verified answers that currently exist only in English (the employee-compliance checklist and the five answers added on September 20).
