...
These are the two test prompts in `metadata.json`. Each ran once at temperature 0 (context 2048) through llama-server on the unmodified Qwen2.5-1.5B-Instruct Q4_K_M file and on the submitted file, with no retrieval, verified answers or system prompt. The full outputs are in `provenance/before_after_outputs.md`. The base file has the stock chat template and the submitted file carries the digest, so the difference shows the fine-tuned weights and the digest together. I did not run the fine-tuned weights without the digest.

**"What are the NSSF contribution rates for an employee earning KSh 30,000 per month?"** The base model gives no figure for KSh 30,000. It lists three invented rates (a "Basic Contribution Rate", a "Contributory Pension Rate" and a "Contributory Provident Fund Rate"), each "typically around 10%". The submitted model states 6% and KES 1,800 for each side, which is correct. It then gives the combined total as KES 3,420 (it should be 3,600) and says no Tier I or Tier II applies (both do, 540 plus 1,260 per side). An earlier temperature-0 run of the same file gave 1,800 per side and 3,600 in total, so temperature-0 output is not exactly reproducible on my setup, and I report this run as obtained.

**"What is the VAT rate?"** The base model does not mention Kenya. It describes VAT in general and lists rates for other countries (United States 0%, European Union 21%, China 13%; the EU has no single rate). The submitted model answers for Kenya and gives the correct standard rate, 16%. It also says the rate applies "on all supplies and services" (some supplies are zero-rated or exempt) and "to top-up an existing KRA PIN account", which is meaningless.

In both cases the answer moves from generic or wrong to Kenya-specific with the right headline figure, but not to a fully correct answer.
...
My first `tp_002` was "What are the basic steps to register a small business in Kenya?". The application answers it from a verified answer, but on the raw file it was unreliable, as above, because the digest has no business-name registration facts. Rewording it did not help: two other phrasings named the Business Registration Service or eCitizen in 0 of 3 runs each. One rewording ("How can I register for my small business in Kenya") at temperature 0 made the model decline ("the material I have available doesn't contain specific registration requirements"), the same deferral I had worked to remove in the first training run.

I then tested nine candidate questions on the submitted file, four runs each (temperature 0, 0.3, 0.3 and 0.8). Eight gave the expected figure in all four runs: VAT threshold, VAT rate, Turnover Tax rate, Corporation Tax, Capital Gains Tax, PAYE top rate with relief, annual leave, and the NSSF prompt. The Turnover Tax range question gave both ends of the range in none. I changed `tp_002` to "What is the VAT rate?".

So both prompts were chosen after testing candidates on the submitted file, and both favour facts the digest states. They do not show how the file handles registration, payroll, SHIF or company-registration questions, which it answered wrongly or inconsistently in my tests. On the NSSF prompt, the per-side figure of KES 1,800 was correct in 9 of the 11 runs I could check. The two wrong ones were a 900/900 split and a Tier II miscalculation at temperature 0.8. The combined total was wrong in one more, and remittance details (usually the 9th working day via iTax, when NSSF is the 9th of the following month through its own portal) were wrong in several. On the VAT prompt the rate 16% appeared in all 17 runs across three tests, while the surrounding text varied: "on all supplies and services" (not correct, since some are zero-rated or exempt), VAT "applied to gross income", and in one run invented food and education rates.
...
| NSSF test prompt, per-side figure of KES 1,800, model file | correct in 9 of 11 runs | none |
| VAT test prompt, 16%, model file | present in all 17 runs | none |
