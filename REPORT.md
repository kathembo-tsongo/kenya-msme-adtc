...
The adapter was merged into the base model with `peft.PeftModel.from_pretrained()` and `merge_and_unload()`, converted to GGUF with llama.cpp's `convert_hf_to_gguf.py`, and quantized to Q4_K_M with `llama-quantize`. The notebook `provenance/My_Offline_AI_advicer_for_kenyan_msmes.ipynb` is included, and there is a Colab link: [colab.research.google.com/drive/1jpcZW0uXTLsfxPQCT2AYnKfoLRiyrdIV](https://colab.research.google.com/drive/1jpcZW0uXTLsfxPQCT2AYnKfoLRiyrdIV?usp=sharing). The notebook in the repository keeps the cell outputs of the submitted run (`global_step=430`, mean training loss 1.3378). I added section and step titles afterwards; the code and outputs are unchanged. Two things in it are not the final file: the data check in Step 2.1 scans the first run's dataset (`training_data_v4_merged.jsonl`), and the digest it writes before quantization (8,620 characters) is the initial one, while the submitted file carries digest v9 (16,915 characters), patched into the quantized file on my laptop. The log of the submitted run is also in `provenance/trainer_state.json` and `provenance/training_log.txt`, and the first run's is in `provenance/original-run/`.
...
The provenance folder holds `adapter_model.safetensors` and `adapter_config.json` for the submitted run, `trainer_state.json`, `training_log.txt`, `dataset_info.md`, `merge_and_quantization.md`, the notebook (whose cell outputs are from the submitted run), and `original-run/` for the first run.
