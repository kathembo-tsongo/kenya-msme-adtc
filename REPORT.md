...
**Raw GGUF (the file the official profiler loads; temperature 0, context 2048, no application layer).** Six of the eight answers came back in English, one was a two-token non-answer ("Kes 98,624.") to the business-registration question, and one was garbled Kiswahili with no useful content (first-employee obligations). Only the turnover-tax answer was clearly correct. In the answers we read, the NSSF question got an invented salary and tier boundary, the SHIF question got a figure of "KES 25,000 per month" instead of 2.75% of gross pay, and the PAYE question got a wrong description of the 25% band. This is consistent with our earlier disclosure that our training data and retrieval corpus are English-only. We therefore do not claim that the raw model answers Kiswahili questions reliably.
